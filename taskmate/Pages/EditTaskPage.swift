...
struct EditTaskPage: View {
    ...
    init(entity: TaskEntity, isNavigate: Binding<Bool>) {
        _viewModel = StateObject(wrappedValue: EditTaskVM(entity: entity))
        _isNavigate = isNavigate
    }
            
    var header: some View {
        VStack(alignment: .leading, spacing: 20) {
            BackButtonView(title: "Edit task") {
                isNavigate = false
            }
            HStack {
                Text("Set Priority")
